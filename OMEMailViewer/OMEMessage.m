...
+ (NSStringEncoding)analizeCharacterSet:(NSString *)charset message:(NSData *)messageData
{
    ...
    if (charset != nil) {
        ...
        switch (detect_code_type((const unsigned char *)[messageData bytes], (unsigned int)[messageData length])) {
            case CT_EUC:
                strEnc = NSJapaneseEUCStringEncoding;
                break;
            case CT_SJIS:
...
- (void)anaData:(NSData *)data
{
    ...
    while ((token = [enumerator nextObject])) {
        if (mstate != MSGSTATE_REAL_MAIL_HEADERS) {
            ...
            NSString *field = [[NSString alloc]initWithData:tokenData encoding:NSASCIIStringEncoding];
...
- (id)initWithContentsOfFile:(NSString *)pathToFile
{
    ...
    if (self != nil) {
        ...
        if ( [[pathToFile pathExtension] caseInsensitiveCompare: @"mpart"] != NSOrderedSame )	{
            targetFilePath = [NSString stringWithString:pathToFile];
            parentPath = [pathToFile stringByDeletingLastPathComponent];
        }
        else	{
            parentPath = [NSString stringWithString:pathToFile];
            ...
            NSUInteger countOfArray = [ extensionArray count ];
            int i;
...
- (NSArray *)incorporateFromExistingFiles
{
    ...
    while ( targetFileName = [dirEnum nextObject] )	{
        ...
        if( !(rule1 && rule2) )	{
            NSMutableDictionary *currentInfo = [NSMutableDictionary dictionaryWithCapacity:1];
            ...
            if ( rule3 )	{			//テキストファイルなら内容をチェック
                ...
                if ( [[NSFileManager defaultManager] fileExistsAtPath: fileInfoFilePath] )	{
                    NSString *fileInfoFileContents = [NSString
                                                      ...
                                                      encoding: NSASCIIStringEncoding
                                                      error:&err];
                    NSUInteger lengthOfFileInfoFileContents = [fileInfoFileContents length];
...
- (NSString *)preferedMIMETypeFromFile:(NSString *)targetFile
{
    ...
    CFStringRef uti = UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension, (__bridge  CFStringRef)currentFileExtension, NULL);
    ...
    NSString *returnValue = [NSString stringWithString: (__bridge  NSString *)prefferdMIMEType];
    CFRelease(prefferdMIMEType);
    return returnValue;
}
...
- (NSStringEncoding)predictTheCharacterSet:(NSString *)filePath
{
    ...
    NSUInteger lengthOriginal = [targetData length];
    ...
    for( NSString *currentCharset in charsetSet )	{
        ...
        NSUInteger thisRate = 0;
        NSUInteger currentScore = 0;
        NSUInteger used = 0;
        BOOL succeed = NO;
        NSData *convertedData;
        //		NSString *convertedString;
        if ( tempString != nil )	{
            NSUInteger initLengthString = [tempString length];
            //unsigned char buffer[initLengthString * 3];
            unsigned char *buffer = malloc(initLengthString * 3);
            succeed = [tempString getBytes:buffer
                            ...
                            remainingRange:	NULL];
            if(succeed) {
                convertedData = [NSData dataWithBytes:buffer length:used];
            }
            free(buffer);
            ...
            NSLog(@"Encoding: %@, score:%lu", currentCharset, (unsigned long)thisRate);
        }
        else
...
- (NSArray *)incorporateFromPartInfoFile
{
    ...
    NSUInteger i, countNodes = [childrenOfRoot count];
               ...
               realHeaders:(NSString **)realHeadersPtr partHeaders:(NSString **)partHeadersPtr
{
    ...
    NSUInteger fileNameLength = [fileNamePtr length];
    NSUInteger i;
    ...
    while( currentExt = [elements nextObject] )	{
        ...
        if ( [myFileManager fileExistsAtPath: currentPath] )	{
            ...
            *fileName = [fnWOExtention stringByAppendingString:currentExt];
            ...
            NSString *newLineString = [ OMEMessage getNextLineString: fileContents ];
            NSUInteger startOfMessagePosition = 0;
            NSUInteger endOfMessagePosition = [fileContents length];
            ...
            if ( rule1.length > 0 )	{
                ...
                NSUInteger stringStartPoint = rule1.location + rule1.length + [newLineString length];
            ...
            if ( rule2.length > 0 )	{
                ...
                NSUInteger stringStartPoint = rule2.location + rule2.length + [newLineString length];
                NSString *tempTop = [NSString stringWithString:[fileContents substringFromIndex:stringStartPoint]];
                //				[tempTop retain];
    ...
    while ( headerName = [ theEnum nextObject ] )	{
        ...
        if ( headerValue != nil )
            [htmlSourceTmp appendString: [self createMailHeaderHTML:headerValue
                                                              label:[headerName stringByAppendingString:@": "]
                                                           idstring:@"header-item-cc"]];
    }
...
- (NSString *)createLinkToFile:(NSString *)fileName inDirectory:(NSString *)fileParent
{
    ...
    [tempHTML appendAsHTMLSource:fileName tag:@"a"
                      attributes:[NSDictionary dictionaryWithObjectsAndKeys:
                                  (__bridge NSString* )CFURLGetString(( CFURLRef)linkURL), @"href", nil]];
    /*
...
- (NSString *)createMailHeaderHTML:(NSString *)value label:(NSString *)label idstring:(NSString *)idstring
{
    ...
    [tempHTML2 setString:@""];
    [tempHTML encloseByTag: @"div" attributes: @{@"class": @"header-data-wrap"}];
    [tempHTML encloseByTag: @"div" attributes: @{@"class": @"header-data"}];
    [tempHTML2 appendAsHTMLSource:label tag: @"div" attributes: @{@"class": @"header-label"}];
    [tempHTML appendString: tempHTML2];
    [tempHTML encloseByTag: @"div" attributes:@{@"id": idstring}];
...
- (NSString *)linkedHTML:(NSString *)source
{
    ...
    NSString *replaced;
    replaced = [regExMail stringByReplacingMatchesInString: source
                                                   options: 0
