...
- (NSString *)tempFolderPath
{
    ...
    if ( aString ) {
        ...
        NSError *error;
        NSURL *aliasURL = [NSURL fileURLWithPath: omeRoot];
		...
		NSString *omeTemp = [resolvedURL.path stringByAppendingPathComponent:@"temp"];
...
- (NSArray *)additionalHeaders
{
	...
	NSUInteger openPos = [paramString rangeOfString: @"{"].location;
	NSUInteger closePos = [paramString rangeOfString: @"}" options:NSBackwardsSearch].location;
	...
	while (item = [enu nextObject] )	{
		NSUInteger startPos = [item rangeOfString: @"\""].location;
		NSUInteger endPos = [item rangeOfString: @"\"" options:NSBackwardsSearch].location;
